...
// Pin definitions for reaction wheel motor
const int PWM_PIN = 5;          // PWM pin for controlling speed
const int IN1_PIN = 7;          // Motor control pin 1 for direction
const int IN2_PIN = 6;          // Motor control pin 2 for direction

// Pin definitions for the bottom motors (left and right)
const int IN1_MOTOR1 = 9;       // Motor 1 control pin 1
const int IN2_MOTOR1 = 8;       // Motor 1 control pin 2
const int PWM_MOTOR1 = 10;      // PWM pin for Motor 1 speed control

const int IN1_MOTOR2 = 13;      // Motor 2 control pin 1
const int IN2_MOTOR2 = 12;      // Motor 2 control pin 2
const int PWM_MOTOR2 = 11;      // PWM pin for Motor 2 speed control

// PID controller variables (outer loop - tilt control)
float Kp_outer = 50.0;          // Proportional gain for outer loop
...
// Feedforward gain (not used in current code)
float feedforwardGain = 1.0;
...
void setup() {
  ...
  // Initialize pins for reaction wheel motor
...
void loop() {
  ...
  if (currentTime - lastTime >= SAMPLE_TIME) {
    ...
    // Compute reaction wheel output (could be modified separately)
    float reactionWheelOutput = desiredVelocity;

    // Set motor speeds for bottom motors and reaction wheel
    setMotorSpeed(bottomMotorOutput, reactionWheelOutput);
...
// Set motor speeds based on PID output for both motor types
void setMotorSpeed(float bottomOutput, float reactionOutput) {
  ...
  int reactionWheelSpeed = map(abs(reactionOutput), 0, 180, MIN_SPEED, MAX_SPEED);
  reactionWheelSpeed = constrain(reactionWheelSpeed, MIN_SPEED, MAX_SPEED);
  ...
  if (reactionOutput > 0) {
    // Set direction for reaction wheel rotation
  ...
  // Set PWM speed for reaction wheel motor
